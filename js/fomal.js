...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | Freezer〃🍈")[0];
}
...
$.ajax({
  ...
  data: {
    key: 'X66BZ-J2G6P-7KRDN-V74FJ-4XGWO-FVBWH',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(103.18773, 26.08294, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Freezer〃🍈 %c 你正在访问Freezer〃🍈の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Freezer〃🍈") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Freezer〃🍈的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("12/30/2022 00:00:00");
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://cdn.skypro.cartafi.cn/2023/04/20/default_cover_1.webp);
    --darkmode-bg:url(https://cdn.skypro.cartafi.cn/2023/04/20/default_cover_69.webp);
